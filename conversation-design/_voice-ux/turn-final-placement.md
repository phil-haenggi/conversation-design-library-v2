...
**Interactional & Psychological Principles**: Recency effect (Murdock, 1962) — the last item in a sequence is recalled most accurately. End-weight principle (Quirk et al., 1985) — longer and more complex elements naturally gravitate to sentence-final position in English. Sequential implicativeness (Schegloff, 2007) — the turn-final element sets up the strongest expectation for what comes next. Transition relevance places (Sacks, Schegloff & Jefferson, 1974) — speakers orient to turn-final elements as the signal for speaker change.
